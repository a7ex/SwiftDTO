...
- generate all files with RESTTranslator (see above)
- in Xcode right click on the folder "GeneratedDTOs" and choose "Add files to "conradkiosk"..."
and select the file corresponding to the entity, which you created, and add it to the project.
- Make sure the new file's "Target Membership" is set to ONLY "ConradDTO"!
...
ALTERNATIVE:
If you have made a large number of changes, just move all contents of "GeneratedDTOs" to the trash, from within Xcode, then generate all files with RESTTranslator (see above) and then add all new files again in Xcode (again making SURE, that the new files "Target Membership" are set to ONLY "ConradDTO"!)
