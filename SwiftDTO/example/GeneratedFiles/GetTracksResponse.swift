...
public struct GetTracksResponse: SessionResponse, DefaultResponse, Information, JSOBJSerializable, DictionaryConvertible, CustomStringConvertible {
    ...
    public let trks: Track?

    // Default initializer:
    public init(sessionValidityDate: Date?, errs: QuantifiableError?, wrngs: QuantifiableWarning?, trks: Track?) {
    ...
    public init?(jsonData: JSOBJ?) {
        ...
        if let val = Track(jsonData: jsonData["trks"] as? JSOBJ) { self.trks = val }
        else { trks = nil }

        #if DEBUG
            DTODiagnostics.analize(jsonData: jsonData, expectedKeys: allExpectedKeys, inClassWithName: "GetTracksResponse")
        #endif
    }

    // all expected keys (for diagnostics in debug mode):
    public var allExpectedKeys: Set<String> {
        return Set(["sessionValidityDate", "errs", "wrngs", "trks"])
    }
    ...
    public var jsobjRepresentation: JSOBJ {
        ...
        if trks != nil { jsonData["trks"] = trks!.jsobjRepresentation }
        return jsonData
    }
