...
public struct Role: JSOBJSerializable, DictionaryConvertible, CustomStringConvertible {

    // DTO properties:
    public let constant: RoleConstant?
    public let name: String?

    // Default initializer:
    public init(constant: RoleConstant?, name: String?) {
    ...
    public init?(jsonData: JSOBJ?) {
        ...
        if let val = RoleConstant.byString(jsonData["constant"] as? String) { self.constant = val }
        else { constant = nil }
        name = stringFromAny(jsonData["name"])

        #if DEBUG
            DTODiagnostics.analize(jsonData: jsonData, expectedKeys: allExpectedKeys, inClassWithName: "Role")
        #endif
    }
