...
//  Copyright (c) 2016 Conrad Electronics SE. All rights reserved.
...
public struct Fish: Animal, JSOBJSerializable, DictionaryConvertible, CustomStringConvertible {
	...
	public func jsonString(paddingPrefix prefix: String = "", printNulls: Bool = false) -> String {
		...
		if let animalType = animalType { returnString = "\(returnString)\t\(prefix)\"animalType\": \("\"\(animalType.rawValue)\""),\n" }
		else if printNulls { returnString = "\(returnString)\t\(prefix)\"animalType\": null,\n" }

		if let numberOfFins = numberOfFins { returnString = "\(returnString)\t\(prefix)\"numberOfFins\": \(numberOfFins),\n" }
		else if printNulls { returnString = "\(returnString)\t\(prefix)\"numberOfFins\": null,\n" }
