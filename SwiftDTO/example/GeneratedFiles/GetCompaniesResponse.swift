...
public struct GetCompaniesResponse: DefaultResponse, Information, JSOBJSerializable, DictionaryConvertible, CustomStringConvertible {
    ...
    // Default initializer:
    public init(errs: QuantifiableError?, wrngs: QuantifiableWarning?, companies: Company?) {
        self.errs = errs
        self.wrngs = wrngs
    ...
    public init?(jsonData: JSOBJ?) {
        ...
        if let val = Company(jsonData: jsonData["companies"] as? JSOBJ) { self.companies = val }
        else { companies = nil }

        #if DEBUG
            DTODiagnostics.analize(jsonData: jsonData, expectedKeys: allExpectedKeys, inClassWithName: "GetCompaniesResponse")
        #endif
    }

    // all expected keys (for diagnostics in debug mode):
    public var allExpectedKeys: Set<String> {
        return Set(["errs", "wrngs", "companies"])
    }
    ...
    public var jsobjRepresentation: JSOBJ {
        ...
        if companies != nil { jsonData["companies"] = companies!.jsobjRepresentation }
        return jsonData
    }
